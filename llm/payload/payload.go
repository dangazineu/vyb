    ...
    walk = func(req *ModuleContextRequest, modulePrefix string) error {
        ...
        if req.ModuleCtx != nil && req.ModuleCtx.GetModuleName() != "" {
            if currentPrefix == "" || currentPrefix == "." {
                ...
                currentPrefix = filepath.Join(currentPrefix, req.ModuleCtx.GetModuleName())
            }
        }

        // Only emit a module header if we have a non-empty path or some context text.
        if currentPrefix != "" || (req.ModuleCtx != nil && (req.ModuleCtx.GetExternalContext() != "" || req.ModuleCtx.GetInternalContext() != "" || req.ModuleCtx.GetPublicContext() != "")) {
